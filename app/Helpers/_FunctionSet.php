...
class FunctionSet
{
    ...
    public function createBrokerOrder($request, $local_broker_id, $order_status, $client_id)
    {
        ...
        Log::debug('FIX API Resquest | New Order Single: ' . $request->client_order_number . ' | ' . $log_text);
        ...
        Log::debug('FIX API Response | New Order Single: ' . $request->client_order_number . ' | ' . $log_text);
        ...
        if ($result_len >0) {
            ...
            $fix_status_result = 'HTTP Response(' . $result['http_code'] . ')';   
            ///Check HTTP Request Status
            if ($result['http_code']!='200') {
                //$data['text'] = 'Order Submission Failed!';
                $data['text'] = 'Communication Failure (HTTP ' . $result['http_code'] . ')';
                $data['status'] = $this->OrderStatus->Failed();
            } else {
                $data['text'] = 'Order Submitted';
                $data['status'] = 'Submitted';
                $fix_status_result = 'HTTP OK (' . $result['http_code'] . ')';
            }
        ...
        if ( $data['status'] = "Failed") {
                ...
                if ($side['fix_value'] === '1') {
                    ...
                    $this->LogActivity->addToLog('Failed Order Funds Released:' . $order_value . '-' . $request->client_order_number);
                }
  ...
  /*            
            ...
            //case "Please Check the endpoint /MessageDownload/Download for message queue":            
                ...
                //$this->LogActivity->addToLog('Order Successful: Please Check the endpoint /MessageDownload/Download for message queue' . '-' . $request->client_order_number);
