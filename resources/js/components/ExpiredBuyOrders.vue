  ...
  <div>
    ...
    <div class="container-fluid" style="margin-top: 10px;">
      <div class="content">
        ...
        <b-card title="Expired BUY Orders">
          <b-form inline>
            <b-pagination
              v-model="currentPage"
              :total-rows="rows"
            ...
            <b-button 
              v-b-modal.modal-1
              @click="create = true">Batch Manual Cancel</b-button>
          </b-form>  
        </b-card>
          <b-table          
            striped
            hover
            show-empty
            :empty-text="'No Expired Buy Orders were found. Try fetching orders for another expiration date and foreign broker.'"
            id="expired-orders"
        ...
        <b-modal id="modal-1" 
          ...
          <form ref="form" @submit.stop.prevent="handleSubmit">
            ...
            <b-button @click="confirmCancelBatchHandler" >Cancel Order(s) Now</b-button>
...
export default {
  ...
  watch: {
    create: function(data) {
      if (data) {
        this.modalTitle = "Manually Cancel Expired Orders";
      } else {
        this.modalTitle = "Manually Cancel Expired Orders";
      }
    },
    processed_orders_count: function(data) {
      if (this.process_status==1) {
        ...
        if (data == this.selected_orders_count) {
            this.$swal("Cancel Batch Completed!", "Expired Order Cancellations Completed!", "success");
  ...
  methods: {
    ...
    async confirmCancelBatchHandler() {
      const result = await this.$swal({
        title: "",
        icon: "info",
        html: `Are you sure you want to <b>Cancel</b> this batch of expired <b>(${this.selected_orders.length})</b> orders? `,
    ...
    async processCancelRequestBatch() {
      this.$swal.fire({
        title: `Cancelling [${this.selected_foreign_broker_name}] orders that expired on [${this.expiring_orders_date}]`,
    ...
    async cancelOrder(id) {
      this.$swal.fire({
        title: `Cancelling [${this.selected_foreign_broker_name}] orders that expired on [${this.expiring_orders_date}]`,
        html: "Processing cancellations, please wait...",
        timerProgressBar: true,
        onBeforeOpen: () => {
          this.$swal.showLoading();
        }
      ...
      try {
        await axios.get(`manual-cancel-er-for/${id}/4`);
        this.cancel_requested_orders.push(id + " Submitted");
        this.processed_orders_count = this.cancel_requested_orders.length;
      } catch (error) {
        this.checkCancelRequestError('Failure Cancelling Expired Order '+id+': ' +error);
      }
    }
  },
  async mounted() {
